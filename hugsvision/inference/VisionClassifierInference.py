...
class VisionClassifierInference:
  ...
  def predict_image(self, img, return_str=True):
    ...
    if return_str:
      print(pred.logits.max(-1)[0].numpy()[0])
      # Get string label from index
      return pred.logits.max(-1)[0].numpy()[0], self.model.config.id2label[predicted_class_idx]
  ...
  img_path: str
  """
  def predict(self, img, return_str=True):  
    return self.predict_image(img, return_str=return_str)
